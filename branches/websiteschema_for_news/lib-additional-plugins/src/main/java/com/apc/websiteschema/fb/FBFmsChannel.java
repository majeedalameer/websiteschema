        ...
        if (null != jobname) {
            ...
            if (null != chnl) {
                ...
                if (StringUtil.isNotEmpty(name)) {
                    doc.addField(tagChannelName, name); // CHANNELNAME
                }
                String apcColumnId = chnl.getApcColumnId();
                addRegalChannel(doc, apcColumnId); // REGALCHANNEL_ONE REGALCHANNEL_TWO REGALCHANNEL_THREE
            ...
            if (null != apcColumns && !apcColumns.isEmpty()) {
                for (FmsApcColumn col : apcColumns) {
                    if ("1".equals(col.getDepth())) {
                        doc.addField(tagApcColumn1, col.getName()); // REGALCHANNEL_ONE
                    } else if ("2".equals(col.getDepth())) {
                        doc.addField(tagApcColumn2, col.getName()); // REGALCHANNEL_TWO
                    } else if ("3".equals(col.getDepth())) {
                        doc.addField(tagApcColumn3, col.getName()); // REGALCHANNEL_THREE
                    }
                }
            }
        }
    ...
    private void addTagOfSource(Doc doc, FmsSource src) {
        if (null != src) {
            ...
            if (StringUtil.isNotEmpty(sourceInfo)) {
                doc.addField(tagSourceInfo, sourceInfo); // SOURCEINFO
            }

            String bigKind = src.getBigKind();
            if (StringUtil.isNotEmpty(bigKind)) {
                doc.addField(tagBigKind, bigKind); // SOURCEBIGKIND
            }

            String smallKind = src.getSmallKind();
            if (StringUtil.isNotEmpty(smallKind)) {
                doc.addField(tagSmallKind, smallKind); // SOURCESMALLKIND
            }
                ...
                if (null != array && array.length > 0) {
                    for (String sc : array) {
                        if (StringUtil.isNotEmpty(sc)) {
                            doc.addField(tagSourceCharacter, sc); // SOURCECHARACTER
                        }
                    }
                }
            }
            ...
            for (FmsRegion region : regions) {
                if ("1".equals(region.getDepth())) {
                    ...
                    if (StringUtil.isNotEmpty(regionName)) {
                        doc.addField(tagSourceRegion1, regionName); // SOURCEREGION_ONE
                    }
                } else if ("2".equals(region.getDepth())) {
                    String regionName = region.getName();
                    if (StringUtil.isNotEmpty(regionName)) {
                        doc.addField(tagSourceRegion2, regionName); // SOURCEREGION_TWO
                    }
                }
            }
        }
