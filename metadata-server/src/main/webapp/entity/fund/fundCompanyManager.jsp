            ...
            Ext.onReady(function(){
                ...
                var store = new Ext.data.JsonStore({
                    ...
                    fields: [
                        ...
                        {name: 'COM_MANAGER', type: 'string'},
                        {name: 'PRE_MANAGER', type: 'string'},
                        {name: 'M_COMMENT', type: 'string'},
                        {name: 'POSITION', type: 'string'}
                    ],
                    // load using script tags for cross domain, if the data in on the same domain as
                    // this page, an HttpProxy would be better
                    proxy: new Ext.data.HttpProxy({
                        url: '/fundComManager/doGetList.action?mapId=ComManager'
                    })
                });
                ...
                var position = new Ext.data.Store({
                    reader: new Ext.data.JsonReader({
                        ...
                        root: 'results',
                        fields: [
                            {name: 'value', mapping: 'PID', type: 'string'},
                            {name: 'display', mapping: 'POSITION', type: 'string'}
                        ]}
                ),
                    autoLoad: true,
                    proxy: new Ext.data.HttpProxy({
                        url: '/fundComManager/doGetList.action?mapId=ComPosition'
                    })
                }
            );
                ...
                function handleEdit(){
                    ...
                    Ext.Ajax.request({
                        url: '/fundComManager/doGetOne.action?mapId=OneComManager', //url to server side script
                        method: 'POST',
                ...
                function rSave(btn, win, panel, action){
                    if(panel.form.isValid()){
                        ...
                        Ext.Ajax.request({
                            url: '/fundComManager/'+action, //url to server side script
                            method: 'POST',
                ...
                function rDelete(btn){
                    ...
                    var request = {json: Ext.util.JSON.encode(selectedKeys)};
                    Ext.Ajax.request({
                        url: '/fundComManager/doDelete.action?mapId=delete', //url to server side script
                        method: 'POST',
                        params: request,//the unique id(s)
                        callback: function (options, success, response){
                            if(success){ //success will be true if the request succeeded
